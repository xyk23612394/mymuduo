...
{
    //下面给channel设置回调，poller给channel通知感兴趣的事件发生，channel会回调相应回调函数
...
//发送数据 应用写的快 而内核发送数据慢，需要把待发送的数据写入缓冲区，而且设置了回调
void TcpConnection::sendInLoop(const void *data,size_t len)
{
    ...
    //之前调用果connection的shutdown，不能再进行发送了
    ...
    //当前这一次write，并没有把全部数据发送出去，剩余的数据需要保存到缓冲区当中，
    // 然后给channel注册epollout事件,poller发现tcp发送缓冲区有空间，会通知相应的sock-channel，调用writeCallback_回调方法
    //也就是调用TcpConnection::handleWrite方法，把发送缓冲区的数据全部发送出去
    {
        //目前发送缓冲区剩余的待发送数据的长度
        ...
        {
            channel_->enableWriting();//这里要注册channel写事件，否则poller不会通知。
        }
    }
}
